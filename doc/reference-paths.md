...
A single place to check a path, a unit name or a package name before writing it
into a document or a runbook. Every row here was read out of the source that
produces it — the file named in the last column is the authority. **If a document
disagrees with this table, this table is right and the document is a bug.**
...
There is **no** `libnss-openbastion`, `nss-openbastion` or
`lightdm-openbastion-greeter` package. `libnss_openbastion.so` ships inside
`open-bastion` (`debian/open-bastion.install`), so `apt install open-bastion`
...
All unit names use the `ob-` prefix. There is no `open-bastion-*.timer` or
...
`ob-cert.socket` and `ob-record.socket` ship disabled (the package cannot know a
host's role); `ob-bastion-setup` enables them, and the Debian `postinst`
re-asserts them on upgrade for hosts that already carry the bastion sshd
drop-in.
