...
| **MT41** | Liaison d'audience sur les jetons `/pam/*`                                                       | R-P1        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#50`        |
| **MT42** | Contrôle d'autorisation intégré sur `/ssh/admin\|certs\|revoke` (refus par défaut)               | R-P2        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#58`        |
| **MT43** | Écriture atomique et verrouillée de la KRL                                                       | R-P3        | Amont        |    P0    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#59`        |
...
| **MT45** | Nettoyage du répertoire temporaire de signature (clé de CA en clair dans `/tmp`)                 | R-P4        | Amont        |    P0    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#60`        |
| **MT46** | Échec fermé sur échec de session synthétique + couverture de tests du plugin device-organization | R-P6        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#71`, `#72` |
...
| **MT48** | Verrouillage réel du magasin de sessions, ou mises à jour par champ avec comparaison-et-échange  | R-P7        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Ouvert** — `#54`, `#66`, `#68`, `#69`       |
| **MT49** | Limitation de débit sur `/ssh/sign` (reverse proxy) et purge des entrées de KRL expirées         | R-P8        | Déploiement  |    P1    | Équipe exploitation | `À COMPLÉTER` | **Ouvert** — `lemonldap-ng-plugins#63`        |
| **MT50** | Figer et déployer une version minimale des quatre plugins                                        | R-P1 à R-P8 | Déploiement  |    P0    | Administrateur LLNG | `À COMPLÉTER` | **Ouvert** — condition d'emploi CE16          |

> **MT48 n'est pas une correction ponctuelle.** Sept courses
> lecture-modification-écriture ont été identifiées sur des magasins qui
> n'offrent aucune atomicité : c'est un **motif**, pas sept bogues. Tant qu'il
> n'est pas traité à la racine, toute nouvelle écriture dans la session
> persistante doit être conçue idempotente et tolérante à l'écrasement.
