...
| **MT46** | Échec fermé sur échec de session synthétique + couverture de tests du plugin device-organization               | R-P6        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Corrigé amont, non publié** — `#72` (échec fermé) et `#71` (PR `#91` : deux fichiers de tests)                     |
...
| **MT48** | Verrouillage réel du magasin de sessions, ou mises à jour par champ avec comparaison-et-échange                | R-P7        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Corrigé amont, non publié** — PR `#87`, `#88`, `#95` ; `#69` corrigé                                               |
| **MT49** | Limitation de débit sur `/ssh/sign` (reverse proxy) et purge des entrées de KRL expirées                       | R-P8        | Déploiement  |    P1    | Équipe exploitation | `À COMPLÉTER` | **Corrigé amont, non publié** — `#63`, PR `#90`                                                                      |
