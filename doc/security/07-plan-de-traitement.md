...
| **MT41** | Liaison d'audience sur les jetons `/pam/*`                                                       | R-P1        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **En revue amont** — `#50`, PR `#92`                                     |
...
| **MT43** | Écriture atomique et verrouillée de la KRL                                                       | R-P3        | Amont        |    P0    | Mainteneur plugins  | `À COMPLÉTER` | **Corrigé amont, non publié** — `#59`                                    |
...
| **MT46** | Échec fermé sur échec de session synthétique + couverture de tests du plugin device-organization | R-P6        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **Partiel** — `#72` corrigé, `#71` ouvert                                |
...
| **MT48** | Verrouillage réel du magasin de sessions, ou mises à jour par champ avec comparaison-et-échange  | R-P7        | Amont        |    P1    | Mainteneur plugins  | `À COMPLÉTER` | **En revue amont** — PR `#87`, `#88` ; `#69` corrigé                     |
| **MT49** | Limitation de débit sur `/ssh/sign` (reverse proxy) et purge des entrées de KRL expirées         | R-P8        | Déploiement  |    P1    | Équipe exploitation | `À COMPLÉTER` | **En revue amont** — `#63`, PR `#90`                                     |
...
| **MT51** | Migrer `ob-bastion-id` de `/pam/bastion-token` (supprimé amont) vers `POST /pam/whoami`          | R-P6        | Produit      |    P0    | Équipe Open Bastion | `À COMPLÉTER` | **Ouvert** — [#246](https://github.com/linagora/open-bastion/issues/246) |
