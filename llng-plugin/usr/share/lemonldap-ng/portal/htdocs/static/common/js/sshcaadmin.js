  ...
  (function () {
      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          data.certificates.forEach(function (cert) {
            ...
            row.innerHTML = '<td>' + (cert.serial || '-') + '</td>' + '<td>' + (cert.user || '-') + '</td>' + '<td><code>' + (cert.principals || '-') + '</code></td>' + '<td>' + formatDate(cert.issued_at) + '</td>' + '<td>' + formatDate(cert.expires_at) + '</td>' + '<td>' + getStatusBadge(cert.status) + '</td>' + '<td>' + getActionButtons(cert) + '</td>';
    ...
    function getActionButtons(cert) {
      if (cert.status === 'revoked') {
        return '<span class="text-muted" title="Revoked by ' + (cert.revoked_by || 'unknown') + ' at ' + formatDate(cert.revoked_at) + '">' + '<span class="fa fa-ban"></span> Revoked</span>';
      }
      return '<button class="btn btn-sm btn-outline-danger btn-revoke" ' + 'data-session-id="' + cert.session_id + '" ' + 'data-serial="' + (cert.serial || '') + '" ' + 'data-user="' + (cert.user || '') + '" ' + 'data-key-id="' + (cert.key_id || '') + '">' + '<span class="fa fa-ban"></span> Revoke</button>';
    }
