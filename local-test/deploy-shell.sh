...
# neither"), and this is the one call site that reports the failure -- throwing
# the message away here leaves the operator with "ob-bastion-id failed" and
# nothing to act on.
BID_ERR="$WORK/bastion-id.err"
BID="$(dssh "${IP[$BASTION_VM]}" 'sudo ob-bastion-id' 2>"$BID_ERR" | tr -d '\r\n')"
if [ -n "$BID" ]; then
