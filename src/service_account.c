...
/*
 * Deliberately the LENIENT parser, unlike config.c which uses
 * str_parse_bool_strict() (#183).
 *
 * The two booleans parsed here — sudo_allowed and sudo_nopasswd — grant
 * privilege when true. Mapping an unrecognised value to false therefore fails
 * in the SAFE direction: a typo such as `sudo = TRUE` denies sudo to a service
 * account instead of granting it. That is the opposite of verify_ssl, where
 * false meant "no TLS verification" and the same typo was fail-open.
 *
 * The failure is also self-announcing (the account visibly cannot sudo), and
 * these files are generated by ob-builder rather than hand-edited, so a strict
 * parser here would buy nothing and could make a service-accounts file that
 * has worked for months suddenly fatal.
 */
